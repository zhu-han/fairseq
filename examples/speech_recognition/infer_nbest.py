...
def nbest_process_predictions(
    ...
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        ...
        if hyp_words_best is None:
            hyp_words_best = hyp_words

        if res_files is not None:
            print(
                "{} ({}-{})".format(hyp_words, speaker, id),
                file=res_files["hypo.words.nbest"],
            )
        ...
        if not args.quiet:
            ...
            logger.info("TARGET:" + tgt_words)
            logger.info("___________________")
    hyp_words_best = hyp_words_best.split()
    tgt_words = tgt_words.split()
    return editdistance.eval(hyp_words, tgt_words), len(tgt_words)
...
def prepare_result_files(args):
    ...
    return {
        ...
        "hypo.words.nbest": get_res_file("hypo.words.nbest"),
    }
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                ...
                errs, length = nbest_process_predictions(
                    args,
                    hypos[i],
                    None,
                    tgt_dict,
