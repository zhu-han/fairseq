...
class KaldiFeatDataset(FairseqDataset):
    def __init__(
        self,
        sample_rate,
        feats_scp_path,
        ...
        **mask_compute_kwargs,
        ...
        with open(feats_scp_path, "r") as f, open(feats_len_path, "r") as fl:
            for i, line in enumerate(f):
                fl_line = fl.readline().strip()
                fl_name, sz =  fl_line.split(" ")[0], int(fl_line.split(" ")[1])                
                items = line.strip().split(" ")
                assert len(items) == 2, line and items[0]==fl_name
                path = items[1]
    ...
    def __getitem__(self, index):
        ...
        feats = kaldi_io.read_mat(path_or_fp)        
        feats = torch.from_numpy(feats).float()
        assert feats.size(-1) == self.feats_dim
    ...
    def collater(self, samples):
        ...
        collated_sources = sources[0].new_zeros(len(sources), target_size, self.feats_dim)
        padding_mask = (
