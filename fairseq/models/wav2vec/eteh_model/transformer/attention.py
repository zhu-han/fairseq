...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, domain_dim=0):
        ...
        self.h = n_head
        self.linear_q = nn.Linear(n_feat + domain_dim, n_feat)
        self.linear_k = nn.Linear(n_feat + domain_dim, n_feat)
        self.linear_v = nn.Linear(n_feat + domain_dim, n_feat)
        self.linear_out = nn.Linear(n_feat + domain_dim, n_feat)
        self.attn = None
    ...
    def forward_attention(self, value, scores, mask, tags=None):
    ...
    def forward(self, query, key, value, mask, tags=None):
        ...
        Args:
            ...
            mask (torch.Tensor): Mask tensor (#batch, 1, time2) or
                (#batch, time1, time2).
            tags (torch.Tensor): Tag tensor (#batch, time1, domain_dim)
        ...
        return self.forward_attention(v, scores, mask, tags)
...
def safe_cumprod(x, *args, **kwargs):
    ...
    shape[-1] = 1
    return torch.cat([torch.ones(shape).to(x.device), exclusive_cumprod], dim=-1)
...
class MTMultiHeadedAttention(MultiHeadedAttention):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, bias_init=0.0, sigmoid_noise=1.0, domain_dim=0):
        """Construct an MultiHeadedAttention object."""
        super().__init__(n_head, n_feat, dropout_rate, domain_dim)
    ...
    def decode_attention(self, value, scores, endpoint, tags=None):
        """Compute attention context vector.
    ...
    def forward(self, query, key, value, mask, tags=None, ep=None):
        """Compute scaled dot product attention.
        ...
        if ep is None:
            ...
            return self.decode_attention(v, score, ep, tags)
